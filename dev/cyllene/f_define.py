    ...
    if expr in FUNCTION_LIST:
        ...
        if expr == 'comp':
            comp = [random.choice([
                    ...
                    for i in range(2)]
            new_expr = sp.sympify(
                fr.set_function_random('comp', comp[0], comp[1]))

        else:
            new_expr = sp.sympify(fr.set_function_random(expr))
...
#             #   ['sanitized' string, compilable flag (boolean), issues list]
        ...
        if check[1]:  
            try:
                new_expr = sp.sympify(
                    check[0], locals=MYLOCALS_SYMPIFY, evaluate=eval_mode)
                if new_expr.is_real:
                    # if input is number, evaluate
                    new_expr = sp.sympify(
                        check[0], locals=MYLOCALS_SYMPIFY, evaluate=True)
                expr_ok = True
                issues = []
